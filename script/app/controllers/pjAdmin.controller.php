...
class pjAdmin extends pjAppController
{
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			$cnt_bookings_today = $pjBookingModel->where("(DATE(t1.created) = DATE(NOW()))")->findCount()->getData();
			$booking_today_arr = $pjBookingModel->reset()->select("t1.*, (SELECT COUNT(`TBS`.service_id) FROM `".$pjBookingServiceModel->getTable()."` AS `TBS` WHERE `TBS`.booking_id=t1.id) AS cnt_services")->where('t1.status', 'confirmed')->where("(DATE(t1.start_dt) = DATE(NOW()))")->findAll()->getData();
			$cnt_services_today = 0;
			foreach($booking_today_arr as $k => $v)
			{
				$cnt_services_today += (int) $v['cnt_services'];
			}
			$cnt_bookings = $pjBookingModel->reset()->findCount()->getData();
			
			$latest_arr = $pjBookingModel->reset()->orderBy("t1.created DESC")->limit(3)->findAll()->getData();
			
			$today_arr = $pjBookingModel->reset()->where('t1.status', 'confirmed')->where("(DATE(t1.start_dt) = DATE(NOW()))")->limit(3)->findAll()->getData();
			
			$this->set('cnt_bookings_today', $cnt_bookings_today);
			$this->set('cnt_services_today', $cnt_services_today);
